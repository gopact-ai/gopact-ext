...
func TestOpen_EmptyDSN(t *testing.T) {
	...
	if err == nil || err.Error() != "mysql: dsn is required" {
		t.Fatalf("Open(\"\") error = %v, want %q", err, "mysql: dsn is required")
	}
}

func TestMigrate_EmptyDSN(t *testing.T) {
	if err := Migrate(""); err == nil || err.Error() != "mysql: dsn is required" {
