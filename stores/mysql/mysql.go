// Package mysql opens a dbstore Store backed by MySQL.
package mysql
...
// Store is the MySQL facade over the shared relational dbstore.
type Store struct {
	*dbstore.Store
}
...
// Open connects to a MySQL database whose Store schema has already been migrated.
func Open(dsn string) (*Store, error) {
	return OpenContext(context.Background(), dsn)
}

// OpenContext connects to a migrated MySQL database using ctx.
