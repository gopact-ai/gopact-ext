...
> **仅支持 Go 1.27+。** 在 Go 1.27.0 正式发布前，请使用 Go 1.27 RC 2 工具链。gopact-ext 现在只发布正式版本。
...
根模块不再充当一次安装全部扩展的聚合包。各领域模块声明的发布版本见
...
编译仓库的 `internal` 发布锚点，同时保持这些包对外不可导入。`PUBLISHED_PREFIX`
表示清单开头已经能从指定模块代理获取的最长连续前缀。更新前部条目时，发布
准备提交先把前缀退回到该条目之前；等新的精确版本可用后再推进。省略前缀时
检查完整清单：
