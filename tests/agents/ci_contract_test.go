...
func TestArkProviderUsesPatchedProtobuf(t *testing.T) {
	...
	if !strings.Contains(goMod, "google.golang.org/protobuf v1.33.0") {
		t.Fatal("models/ark/go.mod must keep google.golang.org/protobuf at patched v1.33.0")
	}
}
...
func TestRepositoryDocumentsCurrentExtensionTags(t *testing.T) {
	...
	for _, install := range []string{
		...
		"go get github.com/gopact-ai/gopact-ext/agents/supervisor@v0.1.1",
...
func TestModuleReadmesDocumentCurrentExtensionTags(t *testing.T) {
	for path, install := range map[string]string{
		...
		"agents/supervisor/README.md":     "go get github.com/gopact-ai/gopact-ext/agents/supervisor@v0.1.1",
