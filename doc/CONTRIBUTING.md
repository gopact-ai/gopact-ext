...
`gopact-ext` is a multi-module Go repository. Each extension must remain independently installable, testable, and releasable. Cross-template tests belong in `tests/agents`; a user should not need every provider or template dependency to adopt one module.

Before opening a pull request, run the commands in `Verification`. Keep provider-specific request shaping inside provider modules, keep agent templates provider-neutral, and keep CI mock-only. Real provider checks are opt-in integration tests using local credentials.
